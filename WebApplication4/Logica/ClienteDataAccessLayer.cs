...
namespace WebApplication4.Logica {
    public class ClienteDataAccessLayer {
        ...
        public void EditarCliente(Cliente pCliente, Administrador pAdministrador) { //Pasar también IP, postTime
            using (SqlConnection con = new SqlConnection(connectionString)) {
                SqlCommand cmd = new SqlCommand("CASP_ModificarCliente", con);
                cmd.CommandType = CommandType.StoredProcedure;

                const string chars = "0123456789";
                string caracteresGenerados = new string(Enumerable.Repeat(chars, 10).Select(s => s[new Random().Next(s.Length)]).ToArray());
                string hashGenerado = caracteresGenerados.GetHashCode().ToString();
                con.Open();
                SqlDataReader rdr = cmd.ExecuteReader();
